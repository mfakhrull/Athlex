...
export function ManageRoundResultsButton({
  ...
  const roundResultsWithMetadata = round.results
    ? addMetadata(round.results)
    : undefined;
    ...
    <>
      ...
      <ManageRoundResultsDialog
        ...
        qualifiedParticipants={participants.filter((p) => {
          // If p.athlete is a string (direct ID)
          if (typeof p.athlete === "string") {
            return round.qualifiedParticipantIds.includes(p.athlete);
          }
          // If p.athlete is an object with _id property
          else if (p.athlete && typeof p.athlete._id === "string") {
            return round.qualifiedParticipantIds.includes(p.athlete._id);
          }
          return false;
        })}
        eventType={eventType}
        onSubmit={async (data) => {
          ...
          try {
            ...
            if (!response.ok) {
              ...
              throw new Error(
                errorData.message || "Failed to update round results"
              );
            }
